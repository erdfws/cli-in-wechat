...
3. **新增多微信账号同时管理 (Multi-Account Support)**：全面支持多个微信账号（通过 npm run acc:add 添加）同时运行在同一个桥接服务中。每个账号享有独立的会话上下文、Token 与运行时状态，大大拓展了使用场景。
4. **完善基础设施与命令管理**：拆分并精简了本地终端 CLI 解析与后台守护进程管理（`account.ts` / `daemon.ts`）；增加并重构了可靠性非常高的多媒体和路由测试用例（覆盖率直达 100% 测试通过）。
